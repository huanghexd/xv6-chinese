...
操作系统的工作是将计算机的资源在多个程序间共享，并且给程序提供一系列比硬件本身更有用的服务。操作系统管理并把底层硬件抽象出来，由此导致，举例来说，一个文字处理软件不用去关心自己使用的是何种硬盘。它也在多个程序之间共享硬件，使得它们可以(至少看起来是)同时运行。最后，操作系统给程序提供一种受控的交互方式，使得程序之间可以共享数据或者协同工作。

操作系统通过接口向用户程序提供服务。设计一个好的接口实际上是很难的。一方面我们希望接口设计得简单和精准，使其易于正确地实现；另一方面，我们可能忍不住想为应用提供一些更加复杂的功能。解决这种矛盾的窍门是让接口的设计依赖于少量的*机制* （*mechanism*)，通过机制的组合来提供强大的通用性。

本书以一个操作系统作为具体的例子来阐述操作系统的概念。这个操作系统就是xv6，它提供了由 Ken Thompson 和 Dennis Ritchie 在Unix操作系统中引入的基本接口，同时完全模仿Unix的内部设计。Unix提供了一个精简的接口，其中的机制配合默契，由此提供了令人吃惊的通用性。这个接口设计得非常成功，使得包括BSD，Linux，Mac OS X，Solaris，甚至在某种程度上包括Microsoft Windows，都提供类似于 Unix 的接口。理解 xv6 是理解这些操作系统和许多其它的操作系统的一个良好开端。
